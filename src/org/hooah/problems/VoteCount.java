...
public class VoteCount {
	...
	public static void main(String[] args) {
		int numCandidates = 4; // how many candidates
		int[] votes = vote(numCandidates);// holds the votes;
		// print the totals:
	...
	private static int[] vote(int numCandidates) {
		int[] votes = new int[numCandidates];// holds the votes;
		...
		{
			int v = new Integer(JOptionPane.showInputDialog("Vote for (0,1,2,3):"));
			if(v >= 0 && v < votes.length) // is it a legal vote?
			{
				votes[v] = votes[v] + 1; } else { processing = false;
			} // quit if there is an illegal vote
		}
		return votes;
	}
}