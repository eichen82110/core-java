...
public class GuessTheWord {
	...
	public void findSecretWord(String[] wordlist, Master master) {
		...
		List<String> copy = Arrays.asList(wordlist);
		List<String> disregard = new ArrayList<>();
		
		System.out.println(copy);
		
		for(int i=0;i<MAX_GUESSES-1;i++){
			String word = copy.get(i);
			if(disregard.contains(word)){
				continue;
			}
			int matches = master.guess(word);
			if(matches==WORD_SIZE){
			...
			if(matches==0){
				...
				for(String w: copy){
					charCount=matches;
					for(char c : word.toCharArray()){
						if(!w.contains(String.valueOf(c))&& (--charCount)==0){
							disregard.add(w);
						}
					}
