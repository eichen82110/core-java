...
/**
 ...
 There are fixed number of items in the home - each with its own weight and value - Jewellery,
 with less weight and highest value vs tables, with less value but a lot heavy.
 To add fuel to the fire, the thief has an old knapsack which has limited capacity.
 Obviously, he can’t split the table into half or jewellery into 3/4ths. He either takes it or leaves it.
		...
		for (int item=1;item<=N;item++){
			...
			for (int weight=1;weight<=W;weight++){
				...
				if (wt[item-1]<=weight){
					...
					V[item][weight]=Knapsack.max (val[item-1]+V[item-1][weight-wt[item-1]], V[item-1][weight]);
				}
				else {
